...
void najgolemKapacitet(SkiCenter *ski, int n)
{
    ...
    for(int i=0;i<n;i++)
    {
        ...
        {
            if(ski[i].liftovi[j].funkcija)
            {
                k[i]+=ski[i].liftovi[j].broj;
            }
        }
        if(k[i]>=max)
        {
    ...
    printf("%s\n%s\n%d\n",ski[index].ime,ski[index].drzava,max);
}
...
int main()
{
	...
	for (i = 0; i < n; i++){
        ...
        for(j=0;j<ski[i].n;j++)
        {
			scanf("%s",ski[i].liftovi[j].ime);
            scanf("%d",&ski[i].liftovi[j].broj);
            scanf("%d",&ski[i].liftovi[j].funkcija);
        }
	}
