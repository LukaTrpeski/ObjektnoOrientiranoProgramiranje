...
public:
    SMS(const char *broj="",double osnovna=0.0)
    {
...
private:
    ...
    static int regular;
public:
    RegularSMS(const char *broj="",double osnovna=0.0,const char *text="",bool roaming=false):SMS(broj,osnovna)
    {
    ...
    {
        int n=0;
        if(strlen(text)%160==0)
            n=strlen(text)/160;
        else
            n=strlen(text)/160+1;
        double temp=osnovna;
        if(roaming)
            temp=temp*(1+regular/100.0);
        else
            temp=temp*(1+danok);
        return n*temp*1.0;
    }
...
private:
    bool humanitarni;
    static int specijalna;
public:
    SpecialSMS(const char *broj="",double osnovna=0.0,bool humanitarni=false):SMS(broj,osnovna)
    {
    ...
    double SMS_cena()
    {
        ...
        if(humanitarni)
            ...
            return temp*(1+specijalna/100.0);
...
void vkupno_SMS(SMS** poraka, int n)
{
    ...
    cout<<"Vkupno ima "<<count1<<" regularni SMS poraki i nivnata cena e: "<<suma1<<endl;
    cout<<"Vkupno ima "<<count2<<" specijalni SMS poraki i nivnata cena e: "<<suma2<<endl;
}
int main(){
