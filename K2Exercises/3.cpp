...
private:
    int i;
public:
    NoCourseException(int k)
    {
        i=k;
    }
    void message()
    {
        cout<<"Demonstratorot so indeks "<<i<<" ne drzi laboratoriski vezbi"<<endl;
    }
};
class Kurs
{
...
protected:
    ...
    int *ocenki;
public:
    Student(int indeks=0,int *ocenki=NULL,int brojOcenki=0)
    {
        this->indeks=indeks;
    ...
    Student(const Student &st)
    {
        ...
        this->brojOcenki=st.brojOcenki;
        this->ocenki=new int[brojOcenki];
        for(int i=0;i<brojOcenki;i++)
        {
            this->ocenki[i]=st.ocenki[i];
        }
    }
...
protected:
    ...
    int brojKursevi;
public:
    Predavach(const char *imeIPrezime="",Kurs *kursevi=NULL,int brojKursevi=0)
    {
    ...
    {
        this->imeIPrezime=new char[strlen(pr.imeIPrezime)+1];
        strcpy(this->imeIPrezime,pr.imeIPrezime);
        this->brojKursevi=pr.brojKursevi;
        for(int i=0;i<brojKursevi;i++)
        {
            this->kursevi[i]=pr.kursevi[i];
        }
    }
...
private:
    int brojchasovi;
public:
    Demonstrator(int indeks=0,int *ocenki=0,int brojOcenki=0,const char *imeIPrezime="",Kurs *kursevi=NULL,int brojKursevi=0,int brojchasovi=0):Student(indeks,ocenki,brojOcenki),Predavach(imeIPrezime,kursevi,brojKursevi)
    {
        this->brojchasovi=brojchasovi;
    }
    int getBodovi()
    {
        try{
        ...
        {
            return Student::getBodovi()+((20*brojchasovi)/brojKursevi);
        }
        }
...
void pecatiDemonstratoriKurs(char* kurs, Student** studenti, int n)
{
    for(int i=0;i<n;i++)
    {
        ...
        if(d)
        for(int k=0;k<d->getBrojKursevi();k++)
        {
            if((*d)[k]==kurs)
            {
                d->pecati();
